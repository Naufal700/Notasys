...
@section('page_title', 'Edit Jenis Akta')
    ...
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
        ...
        <form action="{{ route('master.jenis-akta.update', $jenisAkta->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nama_akta" value="{{ old('nama_akta', $jenisAkta->nama_akta) }}" required
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all">{{ old('deskripsi', $jenisAkta->deskripsi) }}</textarea>
            </div>

            <!-- Buttons -->
            <div class="flex gap-2 pt-3">
                <button type="submit" class="flex-1 bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-3 rounded-xl shadow-lg transform hover:-translate-y-1 transition-all">Update</button>
