...
<div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Template Dokumen</h1>
    ...
    <form action="{{ route('master.template_dokumen.update', $template->id) }}" method="POST" enctype="multipart/form-data" 
          x-data="{ 
              useUpload: {{ $template->file_path != 'templates/default_akta_jual_beli.docx' ? 'true' : 'false' }}, 
              fileName: '{{ $template->file_path != 'templates/default_akta_jual_beli.docx' ? basename($template->file_path) : '' }}',
              checkedActive: {{ $template->is_active ? 'true' : 'false' }}
          }">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="deskripsi" rows="3"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('deskripsi', $template->deskripsi) }}</textarea>
        </div>

        <!-- Jenis Akta -->
        <div class="mb-4">
            <label class="block font-medium text-gray-700 mb-1">Jenis Akta</label>
            <select name="jenis_akta_id"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                <option value="">-- Pilih --</option>
                @foreach(\App\Models\Master\JenisAkta::all() as $akta)
                    <option value="{{ $akta->id }}" {{ old('jenis_akta_id', $template->jenis_akta_id) == $akta->id ? 'selected' : '' }}>
                        {{ $akta->nama_akta }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            <label class="flex items-center cursor-pointer mb-2">
                ...
                <input type="checkbox" x-model="useUpload" class="sr-only" name="use_upload" value="1">
            ...
            <div x-show="!useUpload" class="mt-2 text-gray-600">
                <p>Menggunakan template default sistem (Word/PDF bawaan).</p>
            </div>
        </div>
        ...
        <div class="mb-6 flex items-center">
            <label class="flex items-center cursor-pointer">
                ...
                <div class="relative">
                    <input type="checkbox" name="is_active" x-model="checkedActive" class="sr-only">
        ...
        <div class="flex space-x-3">
            <button type="submit"
                class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">Update</button>
            <a href="{{ route('master.template_dokumen.index') }}"
