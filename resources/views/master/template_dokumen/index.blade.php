...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-auto">
        <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-200">Jenis Akta</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-200">File</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4">{{ $template->nama_template }}</td>
                    <td class="px-6 py-4">{{ $template->jenisAkta?->nama_akta ?? '-' }}</td>
                    <td class="px-6 py-4">
                        @if($template->file_path)
                        <a href="{{ route('template_dokumen.download', $template->id) }}" 
    ...
    <!-- Edit -->
    <a href="{{ route('template_dokumen.edit', $template->id) }}" 
    ...
    <a href="{{ asset('storage/'.$template->file_path) }}" target="_blank" 
       class="text-blue-500 hover:text-blue-600" title="Preview">
       <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Hapus -->
    <form action="{{ route('template_dokumen.destroy', $template->id) }}" method="POST" class="inline">
        @csrf
        @method('DELETE')
